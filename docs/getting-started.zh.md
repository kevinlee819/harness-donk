...
# 成本记账说明：
# - Claude API-key 模式：total_cost_usd 由 Anthropic 直接给出，精确。
# - Claude OAuth/订阅模式：cost 为 null，除非传了 --model
#   （之后按 schema/model-prices.json shadow 折算）。
# - Codex CLI 从不报 USD。若 --model 未设，harness 按 `gpt-5-codex` 价格记账
#   （codex 自己的 exec 默认）。传 --model 可换其他条目（价目表在
#   schema/model-prices.json）。
