...
### 1.2.1 `harness watch` —— 交互式 TUI

curses 实现的仪表盘。终端内三个区：
1. **任务列表**（顶部 ~60%）：实时 1 s 刷新，`▶` 光标，超长自动滚动。
2. **详情区**（中部 ~35%）：当前选中（或📌钉住）任务的 worker `status.json`
...
| `k` / `↑` | 上一个任务 |
| `g` / `G` | 跳首 / 跳尾 |
| `Enter` / `Space` | 📌 把选中任务钉在详情区（再按一次取消） |
| `r` | retry（只接受 `failed` / `merged`） |
| `R` | force-retry（接受孤儿 `working` / `blocked` / `dispatched` / `gating`） |
| `c` | cancel |
| `?` | 帮助覆盖层（任意键关闭） |
| `q` / `Esc` | 退出 |

动作键（`r` / `R` / `c`）通过 shell 调 `harness-task`，DB 仍是唯一真相源。
