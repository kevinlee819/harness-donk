...
```bash
cd ~/code/my-project          # must be a git-initialized directory
...
What `init` does (this step is only run once per new project):

1. Creates `.harness/` at the project root (containing queue, worker state, events, logs, etc.), added entirely to `.gitignore`.
2. Generates `AGENTS.md` at the project root (**you'll be editing this file next**), with the reviewer configured.
...
### 6.1 Configure the Gate — You Must Do This Step

The `AGENTS.md` generated by `init` has a YAML frontmatter section at the top:
