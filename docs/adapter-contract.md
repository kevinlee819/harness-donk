...
ADAPTER_CAP_SESSION_ID_PROGRAMMATIC=0    # Codex: 0；Claude/OpenCode: 1
ADAPTER_CAP_TOOL_PERMISSION=1
ADAPTER_CAP_COST_REPORT=1
ADAPTER_PARALLEL_PER_WORKTREE=0          # Codex: 0（必须串行）
```
...
| Codex | `codex.sh` | `SESSION_ID_PROGRAMMATIC=0`, `PARALLEL_PER_WORKTREE=0` | 必须 flock + 依赖 `--last`；NDJSON 需聚合 |
